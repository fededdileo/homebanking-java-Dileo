...
public class TransactionController {
    ...
    public ResponseEntity<?> transferValidation(
        ...
        return new ResponseEntity<>(toAccount.getOwner().getFirstName() + " " + toAccount.getOwner().getLastName(), HttpStatus.ACCEPTED);
    }
    ...
    public ResponseEntity<Object> transfer(
        ...
        Transaction transactionDebit = new Transaction(TransactionType.DEBIT, amount, "Transferencia Realizada - " + description,LocalDateTime.now(),  accountFrom.getBalance() + amount);
        accountFrom.setBalance(accountFrom.getBalance() - amount);
        accountRepository.save(accountFrom);
        accountFrom.addTransactions(transactionDebit);

        Transaction transactionCredit = new Transaction(TransactionType.CREDIT, -amount, "Successfull Transfer - " + description,LocalDateTime.now(),  accountTo.getBalance() - amount);
        accountTo.setBalance(accountTo.getBalance() + amount);
        accountRepository.save(accountTo);
        accountTo.addTransactions(transactionCredit);


        transactionRepository.save(transactionDebit);
        transactionRepository.save(transactionCredit);
        return new ResponseEntity<>("The transfer was carried out successfully", HttpStatus.ACCEPTED);
    }
    ...
    public ResponseEntity <?> ListTransactionDTO(Authentication authentication, @RequestBody TransactionFilterDTO transactionFilterDTO){
        ...
        List<TransactionDTO> transactionDTOList1 = transactionDTOList.stream().filter(e -> listOfDates.contains(e.getCreationDate().toLocalDate())).collect(Collectors.toList());
        transactionDTOList1.sort(Comparator.comparing(TransactionDTO::getCreationDate).reversed());
